...
/**
 * Class for calculating the mediant of 2 Fractions.
 * 
 * @author Richard Ernst
 */
public class Mediant extends BinaryOperation
{

  /**
   * Computes the mediant of frac1 and frac2.
   * 
  ...
  {
    Integer numerator = frac1.improperNumerator() + frac2.improperNumerator();
  ...
  public List<Step> getSteps(final ExpressionState es, final Style style, final boolean reduced)
  {
    ...
    out.add(new Step(String.format("%s %d + %d = %d", Languages.getAdditionNumDenom(), ln, rn,
        result.improperNumerator()), null, null));
    out.add(new Step(
        String.format("%s %d + %d = %d", Languages.getAdditionNumDenom(), ld, rd, result.denominator()),
        null, null));
